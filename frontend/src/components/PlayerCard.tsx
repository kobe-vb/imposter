...
interface PlayerCardProps {
  ...
  onCommend: (name: string, commendation: string) => void;
}
...
export default function PlayerCard({
  ...
  const handleConfirm = () => {
    ...
    else if (editMode === 'commend') {
      onCommend(player.name, value);
    }
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-4">
        <p
          className={`text-base font-bold truncate ${player.alive ? 'text-white' : 'text-gray-500 line-through'
            }`}
        >
          {player.name}
        </p>
            ...
            <>
              ...
              {adminMode && (
                <>
                  ...
                  <button
                    onClick={() => setEditMode('commend')}
                    className="w-full bg-yellow-600 hover:bg-yellow-700 text-white py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-1"
                  >
                    <Plus className="h-4 w-4" />
                    Commend
                  </button>
